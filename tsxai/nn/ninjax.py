...
def init(fun, **jit_kwargs):
  ...
  def wrapper(*args, **kwargs):
    ...
    # NOTE: Viet: Nevermind, when I changed it to ignore=False, the agent/model breaks => nan loss in the agent
...
@jax.named_scope('jacobian')
def jacobian(fun, targets, has_aux=False):
  """Compute the gradient of an impure function with respect to the specified
  state entries or modules. The transformed function returns a tuple containing
  the computed value, selected state entries, their gradients, and if
  applicable auxiliary outputs of the function."""
  ...
  def wrapper(*args, **kwargs):
    ...
    def forward(x1, x2, *args, **kwargs):
      ...
      changes = {k: v for k, v in state.items() if k in modified}
      return y, (changes, aux)
    backward = jax.jacobian(forward, has_aux=True)

    dx, (changes, aux) = backward(
        x1, x2, *args, seed=seed(None, True), **kwargs)
    if ctx.modify:
      ctx.update(changes)
      x1 = {k: ctx[k] for k in x1.keys()}

    return (x1, dx, aux) if has_aux else (x1, dx)
  return wrapper
...
class Module(object, metaclass=ModuleMeta):
  ...
  def write(self, name, value, ignore_check_axis: int | list | tuple | None = None):
    ...
    # -------------- Shape checking: ignore axis for shape check --------------
    if ignore_check_axis is not None:
      if isinstance(ignore_check_axis, int):
        ignore_check_axis = [ignore_check_axis]
      ignore_check_axis = tuple(ignore_check_axis)
      test_existing_shape = tuple([existing_shape[i] for i in range(len(existing_shape)) if i not in ignore_check_axis])
      test_value_shape = tuple([value_shape[i] for i in range(len(value_shape)) if i not in ignore_check_axis])
    else:
      test_existing_shape = existing_shape
      test_value_shape = value_shape
    assert test_existing_shape == test_value_shape, f"Shape mismatch in the (existing value, overwritten value) combination {(existing_shape, value_shape)}. Mismatching shape: {test_existing_shape} != {test_value_shape}"
    # -------------- End of shape checking --------------
    context()[path] = value
...
class Variable(Module):
  ...
  def __init__(self, make, *args, ignore_check_axis: int | list | tuple | None = None, **kwargs):
    self.make = functools.partial(make, *args, **kwargs)
    self.ignore_check_axis = ignore_check_axis
  ...
  def write(self, value):
    if not self.values:
      self.value('value', self.make)
    return super().write('value', value, ignore_check_axis=self.ignore_check_axis)
...
def flatten(tree, return_keys=False):
  ...
  return (dict(items), treedef, keys) if return_keys else (dict(items), treedef)


def unflatten(mapping, treedef, keys=None):
  if keys is None:
    items = sorted(list(mapping.items()), key=lambda x: x[0])
    _, values = zip(*items) if items else ([], [])
  else:
    values = tuple([mapping[k] for k in keys])
...
def FromHFTransformers(make, initialized=False, postinit=None):

  class HFTransformersModule(Module):
    ...
    def __call__(self, *args, **kwargs):
      if kwargs.get('mutable', False):
        raise NotImplementedError
      params = self._params(*args, **kwargs)
      return self.module.__call__(*args, **kwargs, params=params)
    ...
    def generate(self, *args, **kwargs):
      if kwargs.get('mutable', False):
        raise NotImplementedError
      params = self._params(*args, **kwargs)
      return self.module.generate(*args, **kwargs, params=params)

    def _params(self, *args, **kwargs):
      if self.treedef is None:
        params = self.module.params
        params = postinit(params) if postinit else params
        mapping, self.treedef, self.keys = flatten(params, return_keys=True)
        [self.value(k, v) for k, v in mapping.items()]
      return unflatten(self.values, self.treedef, self.keys)

  return HFTransformersModule
